...
### グリルで確定した追加要件（2026-07-20）

- **ノイズフィルタ**: 自拡張の New Tab ページ（`chrome-extension://<自拡張ID>/` で始まる URL、
  ID は `chrome.runtime.id`）は除外する。`chrome://` 内部ページは Chrome 標準メニューと同様に残す。
- **閉じた時刻の表示**: `session.lastModified`（epoch 秒）を ms に変換し、履歴パネルと同じ
  「日付 時刻」体裁でメタ行に表示する。
- **重複 URL の dedupe**: 同一 URL は**最新のみ**表示する（`getRecentlyClosed()` は新しい順なので
  最初の出現だけ残す）。復元後の再読み込みで同一 URL の古いエントリが繰り上がって再表示されるのは
  正しい挙動として許容する。
- **一覧の鮮度**: 復元後の再読み込みのみ。`visibilitychange` 監視は追加しない（YAGNI。古い一覧でも
  restore は sessionId ベースで誤動作せず、失敗時は再読み込みで実状態に揃う）。
- **リリース**: この機能 PR に version bump を含めない。マージ後に別途 v4.2.0（再審査）とする。
...
- `RecentlyClosedPanel.ts`（+ `index.ts` で re-export）。`HistoryPanel` と同構造:
  ...
  - `activate(): Promise<void>` … タブがアクティブになるたびに
    ...
    （`window` エントリ・自拡張 New Tab は除外、同一 URL は最新のみ）。restore で一覧が変わるため
    毎回再取得する。
  - 各行の描画: favicon（既存 `getFavicon(url)` = `_favicon` API を再利用）+ タイトル
    （`escapeHtml`）+ ドメイン + 閉じた時刻（`lastModified` の日付+時刻）。`data-session-id`
    属性に `tab.sessionId` を保持。
...
- `test/setup.ts` の `mockChrome` に `sessions: { getRecentlyClosed: vi.fn(), restore: vi.fn() }`
  を追加し、`runtime` モックに `id: 'test-id'` を追加（既存モックを壊さない）。
- 新規 `test/recently-closed-panel.test.ts`:
  - タブエントリが一覧描画される（タイトル・sessionId 属性）。
  - `window` エントリが除外される。
  - 自拡張の New Tab（`chrome-extension://test-id/...`）が除外される。
  - 同一 URL は最新のみ表示される（dedupe）。
  - 閉じた時刻（`lastModified`）がメタ行に表示される。
...
- `visibilitychange` によるページ復帰時の自動再読み込み。
- version bump（v4.2.0 はマージ後に別 PR で実施）。
