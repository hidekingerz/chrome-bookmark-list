...
describe('実際のフォルダクリック機能の統合テスト', () => {
  ...
  it('完全な統合テスト: フォルダの展開・折りたたみが正常に動作する', async () => {
    ...
    // 親フォルダのDOM要素を取得
    const parentFolderElement = bookmarkContainer.querySelector('[data-folder-id="2"]') as HTMLElement
    expect(parentFolderElement).toBeDefined()
    
    const parentHeader = parentFolderElement.querySelector('.folder-header') as HTMLElement
    ...
    expect(parentSubfoldersContainer.classList.contains('collapsed')).toBe(true)
    expect(parentSubfoldersContainer.classList.contains('expanded')).toBe(false)
    ...
    expect(parentSubfoldersContainer.classList.contains('expanded')).toBe(true)
    expect(parentSubfoldersContainer.classList.contains('collapsed')).toBe(false)
    expect(parentSubfoldersContainer.style.display).toBe('block')
  })
  ...
  it('ネストしたサブフォルダのクリック機能をテスト', async () => {
    ...
    // サブフォルダを取得
    const subfolderElement = bookmarkContainer.querySelector('[data-folder-id="4"]') as HTMLElement
    expect(subfolderElement).toBeDefined()
    
    const subfolderHeader = subfolderElement.querySelector('.folder-header') as HTMLElement
    const subfolderContainer = subfolderElement.querySelector('.subfolders-container') as HTMLElement
  ...
  it('ブックマークリンククリックのテスト', async () => {
    ...
    // ブックマークリンクを取得
    const bookmarkLink = bookmarkContainer.querySelector('.bookmark-link[data-url="https://github.com"]') as HTMLElement
    ...
    expect(chrome.tabs.create).toHaveBeenCalledWith({ url: 'https://github.com' })
  })
  ...
  it('深くネストしたフォルダの操作テスト', async () => {
    ...
    // 深いネストフォルダのDOM要素を取得
    const deepFolderElement = bookmarkContainer.querySelector('[data-folder-id="6"]') as HTMLElement
    expect(deepFolderElement).toBeDefined()
    expect(deepFolder!.title).toBe('深いネストフォルダ')
    
    // クリックして折りたたみ（初期状態がtrueなので、クリックでfalseになる）
    const deepHeader = deepFolderElement.querySelector('.folder-header') as HTMLElement
    ...
    const deepContainer = deepFolderElement.querySelector('.subfolders-container') as HTMLElement
    if (deepContainer) {
  ...
  it('フォルダが見つからない場合のエラーハンドリング', async () => {
    ...
    // 存在しないフォルダのヘッダーをクリック
    const header = bookmarkContainer.querySelector('.folder-header') as HTMLElement
    const clickEvent = new dom.window.Event('click', { bubbles: true })
  ...
  it('子フォルダをクリックすると親フォルダが折りたたまれる', async () => {
    ...
    const parentElement = bookmarkContainer.querySelector('[data-folder-id="2"]') as HTMLElement
    const parentSubfoldersContainer = parentElement.querySelector('.subfolders-container') as HTMLElement
    expect(parentSubfoldersContainer.classList.contains('expanded')).toBe(true)
    
    // 子フォルダ要素を取得（親のサブフォルダコンテナ内にある）
    const childElement = bookmarkContainer.querySelector('[data-folder-id="4"]') as HTMLElement
    const childHeader = childElement.querySelector('.folder-header') as HTMLElement
    
    expect(childElement).toBeTruthy()
    expect(childHeader).toBeTruthy()
    
    // 子フォルダのサブフォルダコンテナが展開状態であることを確認
    const childSubfoldersContainer = childElement.querySelector('.subfolders-container') as HTMLElement
  ...
  it('サブフォルダがない子フォルダをクリックするとブックマークリストが折りたたまれる', async () => {
    ...
    // 親フォルダのサブフォルダコンテナが展開状態であることを確認
    const parentElement = bookmarkContainer.querySelector('[data-folder-id="parent"]') as HTMLElement
    const parentSubfoldersContainer = parentElement.querySelector('.subfolders-container') as HTMLElement
    expect(parentSubfoldersContainer.classList.contains('expanded')).toBe(true)
    
    // 子フォルダ要素を取得
    const childElement = bookmarkContainer.querySelector('[data-folder-id="child-no-subs"]') as HTMLElement
    const childHeader = childElement.querySelector('.folder-header') as HTMLElement
  ...
  it('親フォルダを折りたたむと、すべての子フォルダが非表示になる', async () => {
    ...
    // DOM要素を取得
    const parentElement = bookmarkContainer.querySelector('[data-folder-id="parent-mixed"]') as HTMLElement
    const parentHeader = parentElement.querySelector('.folder-header') as HTMLElement
    const parentSubfoldersContainer = parentElement.querySelector('.subfolders-container') as HTMLElement
    
    const childWithSubsElement = bookmarkContainer.querySelector('[data-folder-id="child-with-subs"]') as HTMLElement
    const childNoSubsElement = bookmarkContainer.querySelector('[data-folder-id="child-no-subs"]') as HTMLElement
  ...
  describe('3層構造フォルダの動作テスト', () => {
    ...
    it('1層目フォルダクリック時の動作を確認', async () => {
      const layer1Element = document.querySelector('[data-folder-id="100"]') as HTMLElement
      const layer1Header = layer1Element?.querySelector('.folder-header') as HTMLElement
      const layer1Container = layer1Element?.querySelector('.subfolders-container') as HTMLElement
      ...
      const layer2AElement = document.querySelector('[data-folder-id="200"]') as HTMLElement
      const layer2BElement = document.querySelector('[data-folder-id="201"]') as HTMLElement
    ...
    it('2層目フォルダクリック時の動作を確認', async () => {
      const layer2AElement = document.querySelector('[data-folder-id="200"]') as HTMLElement
      const layer2AHeader = layer2AElement?.querySelector('.folder-header') as HTMLElement
      const layer2AContainer = layer2AElement?.querySelector('.subfolders-container') as HTMLElement
    ...
    it('3層目フォルダクリック時の動作を確認', async () => {
      ...
      const layer3Element = document.querySelector('[data-folder-id="300"]') as HTMLElement
      const layer3Header = layer3Element?.querySelector('.folder-header') as HTMLElement
      const layer3BookmarkList = layer3Element?.querySelector('.bookmark-list') as HTMLElement
